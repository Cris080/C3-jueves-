...
/* Spa.
El programa, al recibir como datos el tipo de tratamiento, la edad y el costo
numero de dias internacion de un cliente en un spa, calcula el costo
total del traramiento
...
void main(void)
{
    ...
    if (COS != -1)
    {
        if (EDA >= 60)
            ...
            if (EDA <= 25)
            COS = COS * 0.85;
        printf("\nClave tratamiento: %d\t DIAS: %d\t Costo total: %8.2d\f", TRA, DIA, COS);
    }
    else
